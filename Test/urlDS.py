...
class URLDict:
    ...
    def forceWrite(self,item):
        ...
        if (config.dbUsedforvisitedURLs==0):
            self.visitedURLs[item[0]]=item[1]
            #url:["### ####",self.linkid,<--,links
        else:
            print "db url dict forceWrite query"
...
class currentURLDict:
    ...
    def popitem(self):
        return self.visitingURLs.popitem()
    ...
    def write(self,state,machineNum=""):
        fo = open(config.bkdir+"/"+state+"/curUrlDict"+str(machineNum)+".txt", "w")
        json.dump(self.visitingURLs,fo);
        fo.close()
    
    def recover(self,state,machineNum=""):
        fo = open(config.bkdir+"/"+state+"/curUrlDict"+str(machineNum)+".txt", "r")
    ...
    def getVisitingURLs(self):
        return self.visitingURLs
