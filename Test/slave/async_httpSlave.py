...
class async_httpSlave(asyncore.dispatcher):
    ...
    def __init__(self, host, hostlimited, hostList, preLinkID, depth, path ,slavemap):
        ...
        try:
            ...
            unknownBug(["Occured in first half of init "+str(sys.exc_info()),port,host, hostlimited,hostList, preLinkID, depth, path,linkno])
            return
        ...
        try:
            ...
            writeBug(["HTTP Get Failed "+str(sys.exc_info()),host+":"+port+path,[port,host, hostlimited,hostList, preLinkID, depth, path,linkno]])
...
def slaveSeed(data):
    ...
    for urlStruct in urlStructList:
        ...
        urlDict.add(urlhost+":"+urlport+urlpath,-1,-3) #-2
